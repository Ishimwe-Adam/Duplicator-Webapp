...
const products = [
  { cat: "printing", emoji: "🃏", title: "Business Cards", desc: "Premium full-colour cards. Single or double-sided, gloss or matte, standard or custom sizes.", sku: "SKU-PRT-001", msg: "Business%20Cards", from: 25_000, unit: "per 100 cards" },
  { cat: "signage", emoji: "🎌", title: "PVC Banners", desc: "Durable, weather-resistant PVC banners for outdoor events, storefronts and exhibitions.", sku: "SKU-SGN-002", msg: "PVC%20Banners", from: 95_000, unit: "per roll-up unit" },
  { cat: "sewing", emoji: "👕", title: "Corporate Polo Shirts", desc: "Custom-sewn polo shirts with embroidered or printed logos. Perfect for teams and events.", sku: "SKU-SEW-001", msg: "Corporate%20Polo%20Shirts", from: 28_000, unit: "per piece" },
  { cat: "signage", emoji: "📋", title: "Roll-Up Banners", desc: "Professional retractable roll-up stands for exhibitions, receptions, and presentations.", sku: "SKU-SGN-004", msg: "Roll-Up%20Banners", from: 95_000, unit: "per roll-up unit" },
  { cat: "branding", emoji: "🎁", title: "Branded Mugs & Drinkware", desc: "Ceramic mugs, travel flasks, and glassware printed with your brand for gifting and office use.", sku: "SKU-BRD-003", msg: "Branded%20Mugs", from: 18_000, unit: "per piece" },
  { cat: "gifts", emoji: "🏆", title: "Trophies & Award Plaques", desc: "Engraved trophies, acrylic plaques and crystal awards for corporate recognition events.", sku: "SKU-GFT-001", msg: "Trophies%20and%20Awards", from: 45_000, unit: "per piece" },
  { cat: "printing", emoji: "📰", title: "Letterheads & Stationery", desc: "Official company letterheads, envelopes, compliment slips and full stationery packages.", sku: "SKU-PRT-003", msg: "Letterheads", from: 35_000, unit: "per 100 sheets" },
  { cat: "sewing", emoji: "🦺", title: "Staff Uniforms & Overalls", desc: "Custom-cut and sewn uniforms for hospitality, healthcare, security and industrial teams.", sku: "SKU-SEW-003", msg: "Staff%20Uniforms", from: 35_000, unit: "per piece" },
];
            ...
            {cats.map(cat => (
              <button key={cat} onClick={() => setActiveCat(cat)}
                style={{
                  ...
                  color: activeCat === cat ? (isDark ? "#fff" : "#04091A") : "var(--grey)",
...
function ProductCard({ emoji, title, desc, sku, msg, cat, from, unit }: { emoji: string; title: string; desc: string; sku: string; msg: string; cat: string; from: number; unit: string; }) {
  const [hovered, setHovered] = useState(false);
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <div style={{ padding: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 10, marginBottom: 18, padding: "12px 14px", borderRadius: 10, border: `1px solid ${tc.border}`, background: gd ? "rgba(255,255,255,.03)" : "rgba(38,69,200,.04)" }}>
          <div>
            <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 10, textTransform: "uppercase", letterSpacing: ".1em", color: tc.textMuted, marginBottom: 4 }}>Starting from</div>
            <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 18, fontWeight: 400, color: tc.textPrimary }}>{formatFRW(from)}</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 10, textTransform: "uppercase", letterSpacing: ".1em", color: tc.textMuted, marginBottom: 4 }}>Unit</div>
            <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 12, color: tc.textSecondary }}>{unit}</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
          ...
          <a href={`/login?product=${msg}`}
            style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "9px 18px", background: "#2645C8", color: "#fff", borderRadius: 50, fontFamily: "'Inter', sans-serif", fontWeight: 400, fontSize: 12, textDecoration: "none", transition: "background .2s", textTransform: "uppercase", letterSpacing: ".06em" }}
            onMouseEnter={e => (e.currentTarget.style.background = "#1B2B8A")}
            onMouseLeave={e => (e.currentTarget.style.background = "#2645C8")}
          >
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: "rgba(255,255,255,.85)" }} />
            Order now
          </a>
        </div>
      </div>
